...
pred_score = model.predict(np.array(data_file['video_43']['feature']).reshape(-1,320,1024))
video_info = data_file['video_43']
...
video = cv2.VideoCapture('video/video_43.mp4')
frames = []
...
video_writer = cv2.VideoWriter('static/output-videos/video_43.mp4', cv2.VideoWriter_fourcc(*'MP4V'), 24, keyshots.shape[2:0:-1])
